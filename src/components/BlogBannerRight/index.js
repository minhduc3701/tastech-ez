        ...
        return (
         <Wrapper className="widget">
          <div className="widget-content">
           <PostLink
            style={{backgroundImage: `url(${_.get(posts, '[0].node.featured_media.source_url')})`}}
            to={`${props.langUri}/blog/${posts[0].node.slug}`}
            className="image-hover"
          >
            <AlternativeImage
            imageSrc={_.get(posts, '[0].node.featured_media.source_url')}
          />
          </PostLink>
          </div>
         </Wrapper>
